...
function withItems(order) {
  ...
  return {
    id: order.id, date: order.created_at,
    userId: order.user_id, userName: order.user_name,
    customerId: order.customer_id, customerName: order.customer_name, customerPhone: order.customer_phone,
    subtotal: order.subtotal, tax: order.tax, discount: order.discount, total: order.total,
    status: order.status,
    items: items.map(i => ({
      id: i.product_id, name: i.product_name, emoji: i.product_emoji,
      price: i.price, qty: i.qty, saleType: i.sale_type,
    })),
  };
}
